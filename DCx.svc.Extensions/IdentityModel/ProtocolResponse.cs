...
namespace IdentityModel.Client
{
    ...
    public class ProtocolResponse
    {
        ...
        public static async Task<T> FromHttpResponseAsync<T>(HttpResponseMessage httpResponse, object initializationData = null) where T: ProtocolResponse, new()
        {
                ...
                httpResponse.StatusCode != HttpStatusCode.BadRequest)
            {
                ...
                if (content.IsPresent())
                {
                    try
                    {
                        // (dcx) response.Json = JObject.Parse(content);
                        response.JsonString = content;
                    }
                    catch { }
                }
        ...
        /// The json.
        /// </value>
        // (dcx) public JObject Json { get; protected set; }
        ...
        /// <returns></returns>
        // (dcx) public string TryGet(string name) => Json.TryGetString(name);
        public string TryGet(string oidcConstant) => JsonString.GetOidcValue(oidcConstant);
    }
}